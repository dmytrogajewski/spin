...
func TestCancel_ValidSession_CancelsInProgressExecution(t *testing.T) {
	...
	go func() {
		...
		if err != nil {
			// Error is expected when cancelled
	...
	case resp := <-promptCompleted:
		// Verify stop reason is cancelled
		assert.Equal(t, acp.StopReasonCancelled, resp.StopReason, "Expected cancelled stop reason, but got %s", resp.StopReason)
...
// The provider blocks until the context is cancelled, allowing cancellation tests.
