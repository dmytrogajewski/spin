...
func (g *GitIntegration) Initialize(ctx context.Context) error {
	...
	repo, err := git.Discover(ctx, g.workDir)
	if err != nil {
...
func (g *GitIntegration) GetRepository() *git.Repository {
	g.mu.RLock()
	defer g.mu.RUnlock()
	return g.repo
}
...
func (g *GitIntegration) GetStatus() *git.Status {
	g.mu.RLock()
	defer g.mu.RUnlock()
	return g.lastStatus
}
...
func (g *GitIntegration) GetModifiedFiles() ([]string, error) {
	...
	for _, file := range status.ModifiedFiles {
		if file.Worktree == git.Modified || file.Worktree == git.Added ||
			file.Worktree == git.Deleted || file.Worktree == git.Renamed {
...
func (g *GitIntegration) GetStagedFiles() ([]string, error) {
	...
	var files []string
	for _, file := range status.ModifiedFiles {
		if file.Staging == git.Added || file.Staging == git.Modified ||
			file.Staging == git.Deleted || file.Staging == git.Renamed {
			files = append(files, file.Path)
		}
	}
...
func (g *GitIntegration) GetLog(limit int) ([]git.CommitInfo, error) {
	if !g.IsRepository() {
