...
func TestFileStorage_SaveAndLoad(t *testing.T) {
	...
	// Save
	if err := storage.Save(sessionID, data); err != nil {
...
func TestFileStorage_EmptySessionID(t *testing.T) {
	...
	if err := storage.Save("", HistoryData{}); err == nil {
	...
	if _, err := storage.Exists(""); err == nil {
...
func TestFileStorage_HomeExpansion(t *testing.T) {
	...
	if err := storage.Save("test", testData); err != nil {
	...
	if err := homeStorage.Save("home-test", testData); err != nil {
...
func TestHistory_SaveAndLoad(t *testing.T) {
	...
	if err := history.Save(storage, sessionID); err != nil {
		t.Fatalf("save history: %v", err)
	}

	// Create new history and load
	history2 := NewHistory(4096, &tokenizer.SimpleTokenizer{})
	if err := history2.Load(storage, sessionID); err != nil {
		t.Fatalf("load history: %v", err)
	}
...
func TestHistory_ToFromHistoryData(t *testing.T) {
	...
	history2 := NewHistory(4096, &tokenizer.SimpleTokenizer{})
	if err := history2.FromHistoryData(data); err != nil {
...
func TestFileStorage_AtomicWrite(t *testing.T) {
	...
	if err := storage.Save(sessionID, data1); err != nil {
	...
	if err := storage.Save(sessionID, data2); err != nil {
