...
func (e *TerminalExecutor) Execute(ctx context.Context, cmd tools.CommandInfo, opts interface{}) (tools.ExecutionResult, error) {
	...
	if opts != nil {
		...
		if envMap := extractEnvFromStruct(opts); envMap != nil {
			env = envMap
		} else if envMap, ok := opts.(map[string]interface{}); ok {
			// Check if opts is a map with env vars
			if envVars, exists := envMap["env"]; exists {
				if envSlice, ok := envVars.([]interface{}); ok {
				...
				} else if envMap, ok := envVars.(map[string]interface{}); ok {
...
func extractEnvFromStruct(opts interface{}) []EnvVar {
	if opts == nil {
		return nil
	}
...
func (r *terminalExecutionResult) GetMetadata() map[string]interface{} {
