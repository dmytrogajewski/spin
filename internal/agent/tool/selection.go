...
// Priority defines priority levels for tool selection.
type Priority int
...
const (
	...
	PriorityStaticMCP Priority = 50
	// PriorityDynamicMCP is for dynamically loaded MCP tools.
	PriorityDynamicMCP Priority = 25
)

// SelectionConfig configures dynamic tool selection behavior.
...
// DefaultSelectionConfig returns sensible defaults for tool selection.
...
// Scored represents a tool with its relevance score and priority.
type Scored struct {
	Tool       tools.Tool
	Score      float64
	Priority   Priority
...
// SelectionResult contains the result of tool selection for a turn.
...
// Selector handles dynamic tool discovery and selection.
...
func (s *Selector) SetRuntimeRegistry(registry *tools.Registry) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Selector) collectCandidates(ctx context.Context, query string) []Scored {
...
func (s *Selector) getCoreTools() []Scored {
...
func (s *Selector) searchMCPRegistries(ctx context.Context, query string) []Scored {
...
func (s *Selector) populateDynamicToolInfo(scored *Scored, t tools.Tool) {
...
func (s *Selector) checkIfAlreadyLoaded(scored *Scored, reg mcp.Registry) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Selector) addStickyTools(candidates []Scored) []Scored {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Selector) sortCandidates(candidates []Scored) {
...
func (s *Selector) selectTopTools(candidates []Scored) []Scored {
...
func (s *Selector) groupByServerPath(selected []Scored) map[string][]Scored {
	serverTools := make(map[string][]Scored)
...
func (s *Selector) handleServerLoadError(
	ctx context.Context, err error, serverPath string,
	scoredTools []Scored, result *loadDynamicToolsResult,
) {
	if isOAuthError(err) {
...
func (s *Selector) loadServer(ctx context.Context, serverPath string) ([]tools.Tool, error) {
	s.mu.Lock()
...
func (s *Selector) buildFinalToolList(ctx context.Context, selected []Scored, newlyLoaded []tools.Tool) []tools.Tool {
...
func (s *Selector) addNonLoadableTools(
	ctx context.Context, selected []Scored,
...
func (s *Selector) addAndRegisterNewTools(
...
func (s *Selector) updateStickyTools(selectedTools []tools.Tool) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Selector) emitSelectionEvent(turn int, result *SelectionResult) {
	if s.emitter == nil {
		return
	}
...
func (s *Selector) emitLoadedToolsEvent(loadedTools []tools.Tool) {
	if s.emitter == nil {
		return
	}
	...
	msg := fmt.Sprintf("Loaded %d dynamic tools for this query.", len(loadedTools))
...
func (s *Selector) GetLoadedServers() []string {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Selector) GetStickyTools() []string {
	s.mu.RLock()
	defer s.mu.RUnlock()
