...
func TestACEService_SavePlaybook(t *testing.T) {
	...
	// Save playbook
	if err := svc.SavePlaybook(); err != nil {
...
func TestACEService_BuildPrompt(t *testing.T) {
	...
	if err := svc.playbook.Add(ctx, testBullet); err != nil {
...
func TestACEService_UpdateBullets(t *testing.T) {
	...
	for _, b := range bullets {
		if err := svc.playbook.Add(ctx, b); err != nil {
			t.Fatalf("Failed to add bullet: %v", err)
		}
	}
	...
	// Update bullets
	if err := svc.UpdateBullets(ctx, bullets, fb); err != nil {
