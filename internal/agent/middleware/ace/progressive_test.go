...
func TestProgressiveContextConfig_Defaults(t *testing.T) {
	...
	cfg := acecore.DefaultProgressiveContextConfig()
...
func newMiddlewareWithConfig(cfg acecore.ProgressiveContextConfig) *Middleware {
	return New(nil, &acecore.Config{
		Retrieval: acecore.RetrievalConfig{
			ProgressiveContext: cfg,
		},
	}, nil, nil)
}
...
func TestShouldRetrieveProgressive_Disabled(t *testing.T) {
	...
	mw := newMiddlewareWithConfig(acecore.ProgressiveContextConfig{Enabled: false})

	ctx := trajectory.NewContext("test query")
	ctx.CurrentTurn = 5

	shouldRetrieve, trigger := mw.shouldRetrieve(ctx)
...
func TestShouldRetrieveProgressive_TriggerPriority(t *testing.T) {
	...
	shouldRetrieve2, trigger2 := mw.shouldRetrieve(ctx2)
...
func TestShouldRetrieveProgressive_TurnZero(t *testing.T) {
	...
	mw := newMiddlewareWithConfig(acecore.ProgressiveContextConfig{Enabled: true})
...
// progressiveTriggerCase is a test case for shouldRetrieve trigger detection.
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if shouldRetrieve != tt.wantRetrieve {
				t.Errorf("shouldRetrieve() = %v, want %v", shouldRetrieve, tt.wantRetrieve)
			}
