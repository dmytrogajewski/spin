...
func TestService_Refresh(t *testing.T) {
	...
	// Initial content
	if err := os.WriteFile(agentsPath, []byte("# Version 1"), 0644); err != nil {
		t.Fatalf("failed to create test file: %v", err)
	}
	...
	// Update file
	if err := os.WriteFile(agentsPath, []byte("# Version 2"), 0644); err != nil {
...
func TestService_ThreadSafety(t *testing.T) {
	tempDir := t.TempDir()
	agentsPath := filepath.Join(tempDir, FileName)
	if err := os.WriteFile(agentsPath, []byte("# Concurrent Test"), 0644); err != nil {
		t.Fatalf("failed to create test file: %v", err)
	}
