...
// Config configures the Agentic Context Engineering system.
type Config struct {
	...
	ItemizedLearning ItemizedLearningConfig `mapstructure:"itemized_learning" yaml:"itemized_learning"`
	...
	Refine           RefineConfig           `mapstructure:"refine"            yaml:"refine"`
}

// ConvertConfig converts config.ACEV2 to ace.Config.
// This is needed because the conversation package uses config.ACEV2
// but the ace package uses its own Config type.
func ConvertConfig(v2cfg *config.ACEV2) *Config {
	if v2cfg == nil {
		return nil
	}
...
// RetrievalConfig configures bullet retrieval behavior.
...
// ItemizedLearningConfig configures the ItemizedLearning workflow.
type ItemizedLearningConfig struct {
	Enabled       bool `mapstructure:"enabled"        yaml:"enabled"`
...
// GenerationConfig configures bullet generation (Phase 3+).
type GenerationConfig struct {
	Enabled     bool `mapstructure:"enabled"      yaml:"enabled"`
	AutoReflect bool `mapstructure:"auto_reflect" yaml:"auto_reflect"`
}

// AdapterConfig configures the online learning adapter.
...
// RefineConfig configures playbook refinement and growth management.
...
func (c *ProgressiveContextConfig) Validate() error {
	...
	if c.MaxRetrievalLatencyMs <= 0 {
		errs = append(errs, fmt.Errorf(
			"max_retrieval_latency_ms must be > 0, got %d: %w",
			c.MaxRetrievalLatencyMs, ErrMaxRetrievalLatencyMsMustBe,
		))
	}
...
func (q *QueryWeights) Validate() error {
	...
	if sum < 0.9 || sum > 1.1 {
		errs = append(errs, fmt.Errorf(
			"query weights should sum to approximately 1.0, got %f: %w",
			sum, ErrQueryWeightsShouldSumToApproximately,
		))
	}
