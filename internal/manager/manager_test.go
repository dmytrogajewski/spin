...
func TestManager_buildExecutor(t *testing.T) {
	tests := []struct {
		name    string
		cfg     *Config
		handler security.ApprovalHandler
		wantErr bool
	}{
		{
			name:    "minimal_config",
		...
		},
		{
			...
			cfg:  DefaultConfig(),
			handler: func(req security.ApprovalRequest) security.ApprovalResponse {
				return security.ApprovalResponse{Approved: true, RequestID: req.ID}
			},
			wantErr: false,
		},
		{
