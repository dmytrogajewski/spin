...
func TestDotProduct(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			a:    []float32{1, 2, 3},
			b:    []float32{4, 5, 6},
			want: 32.0, // dot(1,2,3 ; 4,5,6) = 4+10+18.
		},
		{
			name: "orthogonal",
			a:    []float32{1, 0},
...
// Float64 tests added in R13 for generic type parameter support.

func TestCosineSimilarity_Float64(t *testing.T) {
	t.Parallel()
...
func TestDotProduct_Float64(t *testing.T) {
	t.Parallel()

	got := DotProduct([]float64{1, 2, 3}, []float64{4, 5, 6})
	assert.InDelta(t, 32.0, got, floatTolerance)
}

func TestMagnitude_Float64(t *testing.T) {
	t.Parallel()

	got := Magnitude([]float64{3, 4})
	assert.InDelta(t, 5.0, got, floatTolerance)
}

func TestCosineSimilarity_Float64_SelfSimilarity(t *testing.T) {
	t.Parallel()

	vec := []float64{0.5, -0.3, 0.8, 1.2}
