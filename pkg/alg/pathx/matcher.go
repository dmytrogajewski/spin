...
func (m *Matcher) scorePathSegmentMatch(queryLower, pathLower, path string) (score int, indices []int) {
	...
	segments := strings.SplitSeq(dir, string(filepath.Separator))
...
func (m *Matcher) Match(query string, paths []string) []Match {
	...
	// Pre-score all paths into Match structs.
	candidates := make([]Match, 0, len(paths))
	...
	return search.RankedSearch(candidates, func(match Match) float64 {
		return float64(match.Score)
	}, 0, 0)
}
